...
write_cluster_env() {
  ...
  local cp_host ssh_key workers k8s_version pod_cidr ssh_user cni
  ...
  ssh_user="$(k8s_plat_aws_infra_get ssh_user)"
  k8s_version="$(k8s_plat_aws_infra_get kubernetes_version)"
  pod_cidr="$(k8s_plat_aws_infra_get pod_cidr)"
  # Optional: calico (default) or cilium. Empty falls back in kubeadm/lib.sh.
  cni="$(k8s_plat_aws_infra_get cni || true)"
  cat >"${envf}" <<EOF
