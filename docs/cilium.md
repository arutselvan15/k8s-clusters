...
kubeadm:
  ...
  cni: calico                            # or cilium
  calico:
    version: "3.29.3"                    # manifest tag, used when cni: calico
  cilium:
    version: "1.17.18"                   # chart version, used when cni: cilium
    kube_proxy_replacement: true          # Services in eBPF, no kube-proxy
    tunnel_protocol: vxlan                # or geneve
    hubble: true                          # relay + UI
    operator_replicas: 1
```

Both blocks stay pinned, so flipping `cni` needs no other edit — `kubeadm.cilium.*` is simply ignored under `calico`. Every key is optional and validated when the inventory loads, so a typo fails before kubeadm or Helm runs. Defaults live in [`kubeadm/lib.sh`](../scripts/infra/kubeadm/lib.sh).

`kubeadm.cilium.kube_proxy_replacement` is the one value fixed at bootstrap, exactly like `cloud_provider`: it adds `--skip-phases=addon/kube-proxy` to `kubeadm init`, and that phase does not run twice. Changing it — or `cni` itself — on a live cluster means rebuilding the Kubernetes layer (the VMs stay up):
...
- `routingMode=tunnel` — the pod CIDR is not routable on the tenant network, so it has to be encapsulated. Only the protocol is a real choice, which is why `kubeadm.cilium.tunnel_protocol` exists and `routingMode` does not. Calico's IPIP does the same job.
...
`hubble-relay` and `hubble-ui` ship without control-plane tolerations, so on a `worker_nodes: 0` cluster they stay Pending forever — set `kubeadm.cilium.hubble: false` there. That is also why the install gates on `rollout status daemonset/cilium` instead of `helm --wait`, which would block on them.
